...
class AnalysisWorkflowService:
    ...
    async def update_workflow(self, user_id: int, workflow_id: str, update_data: dict[str, Any]) -> AnalysisWorkflow | None:
        """Update a workflow"""
    ...
    async def execute_workflow(self, workflow: AnalysisWorkflow, trigger_analysis: AIAnalysis, execution_type: str = "automatic") -> AnalysisWorkflowExecution:
        ...
        try:
            ...
            await self._send_workflow_notification(workflow, execution, success=True)
            ...
            # Send notification for failed workflow
            await self._send_workflow_notification(workflow, execution, success=False, error=str(e))
    ...
    async def _send_workflow_notification(self, workflow: AnalysisWorkflow, execution: AnalysisWorkflowExecution,
                                        success: bool, error: str | None = None):
        """Send notification for workflow execution completion or failure"""
        try:
            ...
            if success:
                await notification_service.notify_workflow_completed(
                    workflow.user_id,
                    workflow,
                    execution.id,
                ...
                # For failed workflows, use the generic send_notification method
            ...
            logger.error(f"Failed to send notification for workflow {workflow.id}: {str(e)}")
            # Don't raise - notification failure shouldn't fail the workflow execution
