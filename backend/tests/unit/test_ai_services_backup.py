...
class TestAIAnalysisService:
    ...
    @pytest.mark.asyncio
    async def test_create_analysis(self, ai_service, mock_provider, mock_user, db):
        ...
        with patch.object(ai_service, '_queue_analysis_task') as mock_queue:
            with patch('app.services.ai_analysis_service.AIAnalysis') as mock_analysis_model:
                mock_analysis = Mock()
                mock_analysis.id = 1
                mock_analysis.status = "pending"
                mock_analysis_model.return_value = mock_analysis

                analysis = await ai_service.create_analysis(
...
class TestAnalysisScheduler:
    ...
    def scheduler(self):
        ...
        return AnalysisSchedulerService()
